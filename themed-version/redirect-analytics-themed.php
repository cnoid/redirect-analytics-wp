<?php
/**
 * Plugin Name: Redirect Analytics (Themed)
...
function ra_redirect_content($content) {
    if (is_singular('ra_redirects')) {
        ...
        if ($target_url) {
            ...
            // Append the redirecting logic to the content
            $content .= "<div class='redirect-content'><script>
                            setTimeout(function(){
                                window.location.href = '{$target_url}';
                            }, 2000);
                          </script>";
            $content .= "<p>Redirecting in 2 seconds...</p></div>";
        }
    }
    return $content;
}
add_filter('the_content', 'ra_redirect_content');
...
function ra_modify_add_new_admin_link($url, $path) {
    if ($path == 'post-new.php?post_type=ra_redirects') {
        return admin_url('edit.php?post_type=ra_redirects&page=ra_add_redirect');
    }
    return $url;
}
add_filter('admin_url', 'ra_modify_add_new_admin_link', 10, 2);
